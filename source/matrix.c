...
{
     Matrix mTranspose = Matrix_Create(Array_GetDataSize(matrix->array), matrix->column_count, matrix->row_count);
     if (!mTranspose) {
          return NULL;
     }
...
{
     Matrix mVectorize = Matrix_Create(Array_GetDataSize(matrix->array), matrix->row_count * matrix->column_count, 1);
     if (!mVectorize) {
          return NULL;
     }
     for (DS_Size i = 0; i < matrix->row_count; i++) {
          for (DS_Size j = 0; j < matrix->column_count; j++) {
               Matrix_SetData(mVectorize, (i * matrix->column_count) + j, 0, Matrix_GetData(matrix, i, j));
          }
     }
     return mVectorize;
}
