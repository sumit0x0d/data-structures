...
void HashTable_Insert(HashTable *this, const void *key, const void *value)
{
   ...
   index = this->hash_callback.function(key,
      this->bucket_count,
      this->hash_callback.user_data);

   if (!this->pair[index]) {
      this->pair[index] = HashTablePair_Create(key,
         this->key_size, value, this->value_size);
      return;
   }
   ...
   do {
      compare = this->compare_callback.function(key,
         pair->key,
