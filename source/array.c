...
void *Array_SearchLinear(const Array *this,
   const void *data,
   ArrayCompareCallback compare_callback)
{
   ...
   for (i = 0; i < this->capacity; i++) {
      ...
      if (compare_callback.function(current, data, compare_callback.user_data)
         == ARRAY_COMPARE_EQUAL) {
         return current;
      }
   }
...
void *Array_SearchBinary(const Array *this,
   const void *data,
   ArrayCompareCallback compare_callback)
{
   size_t left;
...
void Array_SortBubble(Array *this, ArrayCompareCallback compare_callback)
{
   ...
   for (i = this->capacity - 1; i > 0; i--) {
      for (j = 0; j < i; j++) {
         ...
         if (compare_callback.function(data1, data2, compare_callback.user_data)
            == ARRAY_COMPARE_GREATER) {
